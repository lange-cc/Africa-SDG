...
var url = site_location+'sent/sendmessage';
...
var url = site_location+'sent/Delete/';
...
$('.sms-item').on('click', function(e){
 e.preventDefault();
var id = $(this).attr('data-id');
var url = site_location+'sent/Find/';
   $.ajax({
            type: 'post',
            url: url,
            data : {id: id, name: id} ,
...
$('.loading-panel').css('display','none');
 ...
 var replie_id    = data[i].reply_id;
 var user_replied = data[i].reply_user;


if (user_replied == 'nothing') {
$('.replied-win').hide();
}
else
{
var data1   = JSON.parse(user_replied);
for (var j = 0; j < data1.length; j++) 
{
 var id1        = data1[j].id;
 var names1     = data1[j].names;
 var mail_from1 = data1[j].mail_from;
 var subject1   = data1[j].subject;
 var content1   = data1[j].content;
 var date1      = data1[j].date;
 
 $('#subject1').html(subject1);
 $('#date-time1').html(date1);
 $('#sms-names1').html(names1);
 $('#sms-mail1').html(mail_from1);
 $('#sms-content1').html(content1);

$('.btn-sms-delete1').attr('data-id',id1);
$('.btn-sms-reply').attr('data-id',id1);
$('.btn-sms-reply').attr('data-email',mail_from1);
$('.btn-sms-reply').attr('data-names',names1);
 }
  $('.replied-win').show();
}
...
$('.btn-sms-delete2').attr('data-id',id);
